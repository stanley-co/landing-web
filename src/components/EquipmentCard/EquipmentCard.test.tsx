...
vi.mock('../../contexts/ContactFormModalContext', () => ({ useContactFormModal: () => ({ openModal }) }));
...
describe('EquipmentCard commercial state', () => {
  ...
  it('uses a product-aware quote action and hides promotion without a price', () => {
    openModal.mockReset();
    const host = renderCard({ ...product, promotionText: 'Не показывать' });
    expect(host.textContent).not.toContain('Не показывать');
    act(() => host.querySelector('button')!.click());
    expect(openModal).toHaveBeenCalledWith('Миксер', 'mixer-1');
  });
});
