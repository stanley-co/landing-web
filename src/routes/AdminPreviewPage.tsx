...
export default function AdminPreviewPage() {
  ...
  const buttonEnabled = value.buttonEnabled !== false && (value.actionType === 'OPEN_FORM' || (value.actionType === 'EXTERNAL_LINK' && typeof value.actionValue === 'string' && value.actionValue.trim() !== ''));
  return <IonPage><IonContent><section style={{ padding: 24 }}><img src={url('desktopImageUrl', 'desktopImage')} alt="Предпросмотр слайда" style={{ width: '100%', maxHeight: 480, objectFit: 'cover' }} /><h1>{String(value.title || 'Заголовок слайда')}</h1><p>{String(value.description || 'Описание слайда отсутствует')}</p>{buttonEnabled && <button type="button">{String(value.buttonText || (value.actionType === 'OPEN_FORM' ? 'Оставить заявку' : 'Узнать больше'))}</button>}</section></IonContent></IonPage>;
}
